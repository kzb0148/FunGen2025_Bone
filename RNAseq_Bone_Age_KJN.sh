...
MyID=aubclsd0327          ## Example: MyID=aubtss


WD=/scratch/$MyID/Individual_Component            ## Example:/scratch/$MyID/PracticeRNAseq  
DD=$WD/RawData
RDQ=RawDataQuality
...
REFD=/scratch/Bones_group/BoxerRefGenome          ## Example:/scratch/$MyID/PracticeRNAseq/DaphniaRefGenome    # this directory contains the indexed reference genome for the garter snake
...
RESULTSD=/home/$MyID/Individual_Component/Counts_H_S_2024      ## Example:/home/aubtss/PracticeRNAseq/Counts_H_S
...
fasterq-dump SRR22459524
fasterq-dump DRR378829
fasterq-dump DRR378832
...
ls *.fastq | sed -E 's/_1\.fastq$|_2\.fastq$|\.fastq$//' | sort | uniq > list
...
while read i; do
        ...
        ## STOP & DISCUSS: Check out the trimmomatic documentation to understand the parameters in line 77
                ...
                ## PE for paired end phred-score-type  R1-Infile   R2-Infile  R1-Paired-outfile R1-unpaired-outfile R-Paired-outfile R2-unpaired-outfile  Trimmin$
...
if [[ -f ${DD}/"$i"_1.fastq && -f ${DD}/"$i"_2.fastq ]]; then
        # If both _1 and _2 files exist, run paired-end mode
        echo "Running paired-end trimming for $i"
	java -jar /apps/x86-64/apps/spack_0.19.1/spack/opt/spack/linux-rocky8-zen3/gcc-11.3.0/trimmomatic-0.39-iu723m2xenra563gozbob6ansjnxmnfp/bin/trimmomatic-0.39.jar   \
		 ...
		 ${DD}/"$i"_1.fastq ${DD}/"$i"_2.fastq \
		      ...
		      ${CD}/"$i"_2_paired.fastq ${CD}/"$i"_2_unpaired.fastq \
       	 	ILLUMINACLIP:${DD}/AdaptersToTrim_All.fa:2:35:10 HEADCROP:10 LEADING:30 TRAILING:30 SLIDINGWINDOW:6:30 MINLEN:36

	fastqc ${CD}/"$i"_1_paired.fastq --outdir=${WD}/${PCQ}
	fastqc ${CD}/"$i"_2_paired.fastq --outdir=${WD}/${PCQ}

    # Check if single-end file exists
elif [[ -f ${DD}/"$i".fastq ]]; then
        # If a single file exists (no _1/_2), run single-end mode
        echo "Running single-end trimming for $i"
	java -jar /apps/x86-64/apps/spack_0.19.1/spack/opt/spack/linux-rocky8-zen3/gcc-11.3.0/trimmomatic-0.39-iu723m2xenra563gozbob6ansjnxmnfp/bin/trimmomatic-0.39.jar   \
	...
	fastqc ${CD}/"$i"_trimmed.fastq --outdir="${WD}/${PCQ}"
        ...
        echo "Warning: No input files found for $i"
    fi
...
ls *.fastq | sed -E 's/(_[12]_(paired|unpaired)|_trimmed)\.fastq$//' | sort -u > list	#should list Example: SRR629651
...
if [[ -f ${CD}/"$i"_1_paired.fastq && -f ${CD}/"$i"_2_paired.fastq ]]; then
    echo "Running HISAT2 paired-end alignment for $i"
    hisat2 -p 6 --dta --phred33 \
        -x "${REFD}/Boxer_index" \
        -1 "${CD}/$i"_1_paired.fastq -2 "${CD}/$i"_2_paired.fastq \
        -S "$i".sam

# If not paired-end, check for single-end
elif [[ -f ${CD}/"$i"_trimmed.fastq ]]; then
    echo "Running HISAT2 single-end alignment for $i"
    hisat2 -p 6 --dta --phred33 \
        -x "${REFD}/Boxer_index" \
    ...
    echo "No appropriate input files found for $i"
fi
